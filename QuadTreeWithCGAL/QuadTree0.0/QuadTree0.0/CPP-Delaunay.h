...
// ------------------------------------------------------
__host__ __device__ WORD *BuildTriangleIndexList (void *pointList, float factor, int numberOfInputPoints, int numDimensions, int clockwise, int *numTriangleVertices ) {
...
/* int main(){
   int* ptList = (int*)malloc(10*sizeof(int));
  ...
  WORD *triangleIndexList = BuildTriangleIndexList(ptList, 0, 5, 2, 1, &number_triangles);
  for(int i = 0;i<number_triangles;i++){
     printf("%d ", triangleIndexList[i]);
  }
} */
